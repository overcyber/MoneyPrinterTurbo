...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--transport",
        choices=("native", "api", "docker"),
        default=os.environ.get("MPT_SKILL_TRANSPORT", "native"),
    )
...
def start_docker_api(project_root: Path):
    ...
    log("starting MoneyPrinterTurbo API with Docker provider support")
    try:
        subprocess.run(command, cwd=project_root, check=True)
    except FileNotFoundError as exc:
        raise SkillRunnerError("docker is not installed") from exc
...
def download_video(base_url, api_key, value, output):
    ...
    try:
        with urllib.request.urlopen(request, timeout=600) as response, output.open("wb") as f:
...
def run_api(args):
    ...
    if args.image_provider:
        ...
        log(f"image provider healthy: {args.image_provider} ({status.get('elapsed_ms', 'n/a')} ms)")
...
if __name__ == "__main__":
    try:
        ...
        print("MPT_ERROR", file=os.sys.stderr)
